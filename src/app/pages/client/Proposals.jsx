...
export default function Proposals() {
  ...
  const [resolvedRequestData, setResolvedRequestData] = useState(null);
  ...
  const effectiveRequestData = requestData || resolvedRequestData;
  const clientBudgetMin = Number(
    effectiveRequestData?.rawBudgetMin ?? effectiveRequestData?.budgetMin ?? effectiveRequestData?.BudgetMin ?? 0,
  );
  const clientBudgetMax = Number(
    effectiveRequestData?.rawBudgetMax ?? effectiveRequestData?.budgetMax ?? effectiveRequestData?.BudgetMax ?? 0,
  );
  const clientDeadline =
    effectiveRequestData?.rawExpectedDeadline ?? effectiveRequestData?.expectedDeadline ?? effectiveRequestData?.ExpectedDeadline ?? '';
  const requestTitle = effectiveRequestData?.title || `Request ${requestId}`;
  ...
  useEffect(() => {
    let cancelled = false;
    (async () => {
      if (!user?.token || !requestId || requestData) return;
      try {
        const pageSize = 20;
        const requestStatusVariants = [undefined, 1, 2, 3];
        let match = null;

        for (let s = 0; s < requestStatusVariants.length && !match; s += 1) {
          const requestStatus = requestStatusVariants[s];
          let pageIndex = 1;
          let totalPages = 1;

          while (pageIndex <= totalPages && !match) {
            ...
            match = rows.find((row) => String(row?.id ?? row?.Id ?? '') === String(requestId)) || null;
        ...
        setResolvedRequestData({
          title: match.title ?? match.Title ?? `Request ${requestId}`,
          rawBudgetMin: Number(match.budgetMin ?? match.BudgetMin ?? 0),
          rawBudgetMax: Number(match.budgetMax ?? match.BudgetMax ?? 0),
          rawExpectedDeadline: match.expectedDeadline ?? match.ExpectedDeadline ?? '',
        });
      } catch {
        /* Keep fallback request values if request details are unavailable */
      }
    })();
    return () => {
      cancelled = true;
    };
  }, [user?.token, requestId, requestData]);
