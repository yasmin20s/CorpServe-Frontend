        ...
        <div className="hidden items-center gap-3 sm:gap-5 md:flex">
          {navLinks.map((item) => (
            <a
              key={item.id}
              href={`#${item.id}`}
              className={`text-sm font-semibold capitalize transition-colors sm:text-base ${activeSection === item.id ? 'text-[#6f74ea] dark:text-indigo-300' : 'text-slate-700 hover:text-[#6f74ea] dark:text-slate-200 dark:hover:text-indigo-300'
                }`}
            >
              {item.label}
            </a>
          ...
          <div className="flex flex-col gap-2">
            {navLinks.map((item) => (
              <a
                ...
                className={`rounded-md px-3 py-2 text-sm font-semibold capitalize transition-colors ${activeSection === item.id
